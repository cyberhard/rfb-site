...
export async function GET(req: Request) {
  try {
    ...
    if (!code_verifier) return NextResponse.json({ error: "Missing verifier" }, { status: 400 });
    ...
    if (!tokenRes.ok) {
      return NextResponse.json({ error: data.error || "VK token error" }, { status: 400 });
    }

    return NextResponse.json({ success: true, vk: data });
